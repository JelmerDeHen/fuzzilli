function main() {
const v3 = "object".startsWith("object",-1815729407);
// v3 = .boolean
const v6 = new Uint16Array(39101);
// v6 = .object(ofGroup: Uint16Array, withProperties: ["length", "byteLength", "__proto__", "buffer", "constructor", "byteOffset"], withMethods: ["fill", "findIndex", "filter", "entries", "map", "find", "sort", "every", "reduce", "join", "set", "copyWithin", "subarray", "reverse", "some", "indexOf", "includes", "reduceRight", "lastIndexOf", "values", "slice", "keys", "forEach"])
const v8 = Symbol.search;
// v8 = .object(ofGroup: Symbol, withProperties: ["__proto__", "description"])
v8[v8] = v6;
const v10 = [13.37];
// v10 = .object(ofGroup: Array, withProperties: ["length", "__proto__", "constructor"], withMethods: ["copyWithin", "reduceRight", "concat", "every", "some", "toString", "filter", "toLocaleString", "slice", "map", "join", "values", "sort", "indexOf", "keys", "splice", "fill", "includes", "pop", "reduce", "lastIndexOf", "find", "flatMap", "findIndex", "unshift", "flat", "reverse", "shift", "entries", "forEach", "push"])
const v12 = [1337,1337];
// v12 = .object(ofGroup: Array, withProperties: ["length", "__proto__", "constructor"], withMethods: ["copyWithin", "reduceRight", "concat", "every", "some", "toString", "filter", "toLocaleString", "slice", "map", "join", "values", "sort", "indexOf", "keys", "splice", "fill", "includes", "pop", "reduce", "lastIndexOf", "find", "flatMap", "findIndex", "unshift", "flat", "reverse", "shift", "entries", "forEach", "push"])
const v13 = [-1815729407,13.37,"object",Reflect];
// v13 = .object(ofGroup: Array, withProperties: ["length", "__proto__", "constructor"], withMethods: ["copyWithin", "reduceRight", "concat", "every", "some", "toString", "filter", "toLocaleString", "slice", "map", "join", "values", "sort", "indexOf", "keys", "splice", "fill", "includes", "pop", "reduce", "lastIndexOf", "find", "flatMap", "findIndex", "unshift", "flat", "reverse", "shift", "entries", "forEach", "push"])
const v14 = {a:"object",__proto__:v13,e:Reflect};
// v14 = .object(ofGroup: Object, withProperties: ["e", "__proto__", "a"])
const v15 = {d:Reflect,e:1337,toString:v12,length:v12,b:v10};
// v15 = .object(ofGroup: Object, withProperties: ["length", "toString", "__proto__", "d", "e", "b"])
let v16 = v15;
const v19 = new Int8Array("arguments");
// v19 = .object(ofGroup: Int8Array, withProperties: ["__proto__", "buffer", "byteOffset", "constructor", "length", "byteLength"], withMethods: ["some", "fill", "copyWithin", "values", "reduceRight", "lastIndexOf", "findIndex", "slice", "join", "indexOf", "reverse", "sort", "forEach", "map", "find", "subarray", "reduce", "every", "keys", "includes", "entries", "filter", "set"])
const v21 = new Map(v19);
// v21 = .object(ofGroup: Map, withProperties: ["size", "__proto__"], withMethods: ["has", "keys", "get", "forEach", "entries", "set", "values", "clear", "delete"])
const v22 = v21.values(v19);
// v22 = .object()
function v25(v26,v27) {
    for (const v29 in "boolean") {
        const v31 = {};
        // v31 = .object(ofGroup: Object, withProperties: ["__proto__"])
        const v32 = [v31,v31,v31,v31,v31];
        // v32 = .object(ofGroup: Array, withProperties: ["length", "__proto__", "constructor"], withMethods: ["copyWithin", "reduceRight", "concat", "every", "some", "toString", "filter", "toLocaleString", "slice", "map", "join", "values", "sort", "indexOf", "keys", "splice", "fill", "includes", "pop", "reduce", "lastIndexOf", "find", "flatMap", "findIndex", "unshift", "flat", "reverse", "shift", "entries", "forEach", "push"])
        v26[3729928015] = v22;
        return 8;
        function v37(v38,v39) {
            let v40 = Map;
            return 3;
        }
        let v41 = 0;
        do {
            let v44 = 0;
            while (v44 < 5) {
                const v45 = v44 + 1;
                // v45 = .primitive
                v44 = v45;
            }
            const v46 = v41 + 1;
            // v46 = .primitive
            v41 = v46;
        } while (v41 <= 8);
    }
    return "object";
}
const v48 = [1337];
// v48 = .object(ofGroup: Array, withProperties: ["length", "__proto__", "constructor"], withMethods: ["copyWithin", "reduceRight", "concat", "every", "some", "toString", "filter", "toLocaleString", "slice", "map", "join", "values", "sort", "indexOf", "keys", "splice", "fill", "includes", "pop", "reduce", "lastIndexOf", "find", "flatMap", "findIndex", "unshift", "flat", "reverse", "shift", "entries", "forEach", "push"])
let v49 = v48;
const v50 = v25(v49);
// v50 = .unknown
const v51 = v25(13.37);
// v51 = .unknown
let v54 = 0;
while (v54 < 5) {
    const v55 = v54 + 1;
    // v55 = .primitive
    v54 = v55;
}
v50.__proto__ = "boolean";
const v56 = v25(v19);
// v56 = .unknown
const v58 = Symbol.iterator;
// v58 = .object(ofGroup: Symbol, withProperties: ["__proto__", "description"])
v22[v58] = v25;
const v59 = Symbol.for(v56);
// v59 = .object(ofGroup: Symbol, withProperties: ["__proto__", "description"])
const v60 = Int8Array in Map;
// v60 = .boolean
const v61 = Symbol(v51);
// v61 = .object(ofGroup: Symbol, withProperties: ["__proto__", "description"])
}
%NeverOptimizeFunction(main);
main();
